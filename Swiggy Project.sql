SELECT * FROM fooo.swiggy;
#Q1. HOW MANY RESTAURANTS HAVE A RATING GREATER THAN 4.5?
select count(distinct restaurant_name) as highest_rating_Restuarant from swiggy 
where rating > 4.5;

#Q2.WHICH IS THE TOP 1 CITY WITH THE HIGHEST NUMBER OF RESTAURANTS?
select city, count(distinct restaurant_name) 
as restuarant_count from swiggy 
group by city 
order by restuarant_count desc
limit 1;
...
#Q11 LIST THE NAME OF THE 	RESTAURANT ARE 100% VEGETARIAN IN ALPHABETICAL ORDER OF RESTAURANT NAME.
select distinct restaurant_name,
...
select distinct restaurant_name,
(count(case when veg_or_nonveg='Non-veg' then 1 end)*100/count(*)) as nonvegetarian_percentage from swiggy
group by restaurant_name
